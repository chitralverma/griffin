 ...
 * @param appConfig    dq measurement configuration (must)
 */
@JsonInclude(Include.NON_NULL)
case class GriffinConfig(
    @JsonProperty("env") envConfig: EnvConfig,
    @JsonProperty("dq") appConfig: AppConfig)
    extends Param {

  def validate(): Unit = {
    assert(envConfig != null, "environment config should not be null")
    assert(appConfig != null, "dq config should not be null")
    envConfig.validate()
    appConfig.validate()
  }
}
